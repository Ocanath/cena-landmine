...
schtasks /query /tn "KeyboardAudioPlayer" >nul 2>&1
if %errorLevel% neq 0 (
    echo Task "KeyboardAudioPlayer" does not exist.
...
echo Removing scheduled task "KeyboardAudioPlayer"...
echo.

REM Stop the task if it's running
echo Stopping task if running...
schtasks /end /tn "KeyboardAudioPlayer" >nul 2>&1

REM Delete the task
schtasks /delete /tn "KeyboardAudioPlayer" /f
...
echo   Look for "KeyboardAudioPlayer.exe" under Background processes
echo.
pause
